...
if __name__ == "__main__":
    ...
    # Compute the average accuracy for is_assistive_object == True
    acc_assistive = data[data["is_assistive_object"] == True]["acc"].mean()
    # Compute the average accuracy for is_assistive_object == False
    acc_non_assistive = data[data["is_assistive_object"] == False]["acc"].mean()
    # Print the results
    print(f"Average accuracy for is_assistive_object == True: {acc_assistive}")
    print(f"Average accuracy for is_assistive_object == False: {acc_non_assistive}")
    for question_type in ["O", "D", "S", "A"]:
        print(f"\nQuestion Type = {question_type}")
        # Compute the average accuracy for is_assistive_object == True
        acc_assistive = data[(data["is_assistive_object"] == True) & (data["question_type"] == question_type)]["acc"].mean()
        # Compute the average accuracy for is_assistive_object == False
        acc_non_assistive = data[(data["is_assistive_object"] == False) & (data["question_type"] == question_type)]["acc"].mean()
        # Print the results
        print(f"Average accuracy for is_assistive_object == True: {acc_assistive}")
        print(f"Average accuracy for is_assistive_object == False: {acc_non_assistive}")
